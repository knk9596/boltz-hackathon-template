...
#to cluster the multiple (>5) outputs
...
def post_process_protein_ligand(datapoint: Datapoint, input_dicts: List[dict[str, Any]], cli_args_list: List[list[str]], prediction_dirs: List[Path]) -> List[Path]:
    ...
    for pdb_file in all_pdbs:
        ...
        ligand_atoms = [atom for atom in structure.get_atoms() 
                        if atom.get_parent().get_parent().id == 'B']
        ...
        for lig_atom in ligand_atoms:
            close_atoms = ns.search(lig_atom.coord, 5.0)
            for atom in close_atoms:
                residue = atom.get_parent()
                interacting_residues.add((residue.get_parent().id, residue.id[1]))
        
        interaction_fingerprints.append(interacting_residues)
    
    # Create binary fingerprint matrix
    all_residues = sorted(set().union(*interaction_fingerprints))
    fingerprint_matrix = np.zeros((len(interaction_fingerprints), len(all_residues)))
    for i, fp in enumerate(interaction_fingerprints):
        for j, res in enumerate(all_residues):
            if res in fp:
                fingerprint_matrix[i, j] = 1
    
    # Calculate distance matrix and cluster
    jaccard_matrix = pairwise_distances(fingerprint_matrix, metric='jaccard')
    clustering_dbscan = DBSCAN(eps=0.3, min_samples=2, metric='precomputed')
    labels = clustering_dbscan.fit_predict(jaccard_matrix)
    
    unique_labels = [l for l in set(labels) if l != -1]
    
    # If only one cluster, randomly select 5
    if len(unique_labels) <= 1:
        import random
        random.seed(42)
        selected_pdbs = random.sample(all_pdbs, min(5, len(all_pdbs)))
        return sorted(selected_pdbs)
    
    # Multiple clusters: skip largest, take representatives from others
    cluster_sizes = {label: sum(1 for l in labels if l == label) for label in unique_labels}
    largest_cluster = max(cluster_sizes, key=cluster_sizes.get)
    
    selected_indices = []
    
    # Get representatives from non-largest clusters
    for cluster_id in unique_labels:
        if cluster_id == largest_cluster:
            continue
        
        cluster_indices = [i for i, l in enumerate(labels) if l == cluster_id]
        cluster_distances = jaccard_matrix[np.ix_(cluster_indices, cluster_indices)]
        avg_distances = cluster_distances.sum(axis=1)
        medoid_local_idx = avg_distances.argmin()
        medoid_global_idx = cluster_indices[medoid_local_idx]
        
        selected_indices.append(medoid_global_idx)
    
    # Add more from non-largest clusters until we have 5
    for cluster_id in unique_labels:
        if cluster_id == largest_cluster:
            continue
    ...
    # If still not enough, add from largest cluster
    if len(selected_indices) < 5:
        largest_indices = [i for i, l in enumerate(labels) if l == largest_cluster]
    ...
    selected_pdbs = [all_pdbs[i] for i in selected_indices[:5]]
